...
@export var choices_file_path: String = "res://dialoglar/choices.json"
var messages: Array = []
...
func _ready():
	...
	test_btn.text = "TEST BUTONU"
	test_btn.position = Vector2(50, 50)
	test_btn.size = Vector2(200, 60)
	test_btn.pressed.connect(func(): print("TEST BUTONU ÇALIŞIYOR!"))
	add_child(test_btn)
	
	# Seçim geçmişini yükle
	load_choices_from_file()
	
	print("Scene hazır, test butonu eklendi")
...
func get_messages(file_path: String) -> Array:
	...
	for msg in data:
		if not msg is Dictionary or not "text" in msg or not "who" in msg:
			...
			return []
		if "choices" in msg:
			if not msg.choices is Array:
				printerr("Invalid choices format in message: ", msg)
				return []
			for choice in msg.choices:
				if typeof(choice) == TYPE_STRING:
					continue
				if not choice is Dictionary or not "text" in choice or not "next_index" in choice:
					printerr("Invalid choice format: ", choice)
					return []
...
func show_regular_message(msg_data: Dictionary):
	var message_container = create_message_bubble(msg_data.text, msg_data.who)
...
func show_choice_message(msg_data: Dictionary):
	...
	await get_tree().create_timer(UI_CHOICE_DELAY).timeout
...
func create_choice_buttons(choices: Array):
	...
	for i in range(choices.size()):
		var choice_text = choices[i].text if typeof(choices[i]) == TYPE_DICTIONARY else choices[i]
		...
		var idx = i
		var txt = choice_text
		button.pressed.connect(func(): _on_choice_button_clicked(idx, txt))
		button.mouse_entered.connect(func(): print("MOUSE GİRDİ: ", idx))
		button.mouse_exited.connect(func(): print("MOUSE ÇIKTI: ", idx))
...
func _on_choice_button_clicked(choice_index: int, choice_text: String):
	...
	if current_choices[choice_index] is Dictionary and "next_index" in current_choices[choice_index]:
		next_index = current_choices[choice_index].next_index
		if next_index >= 0 and next_index < messages.size():
...
func save_player_choice(dialog_id: int, choice_index: int, choice_text: String):
	...
	print("Seçim kaydedildi - Dialog ", dialog_id, ": ", choice_text)
	save_choices_to_file()

func save_choices_to_file():
	var file = FileAccess.open(choices_file_path, FileAccess.WRITE)
	if file != null:
		var json_string = JSON.stringify(player_choices)
		file.store_string(json_string)
		file.close()
		print("Seçimler dosyaya kaydedildi")
	else:
		printerr("Seçimler kaydedilemedi: ", choices_file_path)

func load_choices_from_file():
	var file = FileAccess.open(choices_file_path, FileAccess.READ)
	if file != null:
		var json_string = file.get_as_text()
		file.close()
		var json = JSON.new()
		if json.parse(json_string) == OK:
			player_choices = json.data
			print("Seçimler yüklendi: ", player_choices)
		else:
			printerr("Seçimler yüklenemedi - JSON hatası")
	else:
		print("Seçim dosyası bulunamadı")

func get_player_choice(dialog_id: int):
	if dialog_id in player_choices:
		return player_choices[dialog_id]
	return null
